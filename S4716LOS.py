...
#The radial velocity is from the theory given by
#The values obtained from the kepler fit will be used
#v_r=np.sin(IS2real)*(dr1(tpreal,tid,Mbhreal,areal,ereal)*np.sin(oomegareal+theta1(tpreal,tid,Mbhreal,areal,ereal))+keprad(tpreal,tid,Mbhreal,areal,ereal)*np.cos(oomegareal+theta1(tpreal,tid,Mbhreal,areal,ereal))*dt2(tpreal,tid,Mbhreal,areal,ereal))

tid=np.linspace(0,19.99,2000)
#print('tid =',tid)
...
#brug disse gæt til at bestemme angular momentum
dthetaS2g = dt2(tpreal, 0, Mbhreal, areal, ereal)
perS2g = keprad(tpreal, 0, Mbhreal, areal, ereal)

#It doesn't matter if we use angular momentum in the formula or the use the normal formula without angular momentum.
#It gives the same result.
#Both functions are in the loop below. Only choose to use one of them.

radvelplot=[]
for i in range(2000):
    dt_vr=i*10/1000
    radvelplot.append(np.sin(IS2real)*(dr1(tpreal,dt_vr,Mbhreal,areal,ereal)*np.sin(oomegareal+theta1(tpreal,dt_vr,Mbhreal,areal,ereal))
                     +keprad(tpreal,dt_vr,Mbhreal,areal,ereal)*np.cos(oomegareal+theta1(tpreal,dt_vr,Mbhreal,areal,ereal))*dt2(tpreal,dt_vr,Mbhreal,areal,ereal))
                      * (3.0857*10**13) * 1/(365*24*60*60))
#    radvelplot.append(np.sin(IS2real)*(dr1(tpreal,dt_vr,Mbhreal,areal,ereal)*np.sin(oomegareal+theta1(tpreal,dt_vr,Mbhreal,areal,ereal))
#                     +keprad(tpreal,dt_vr,Mbhreal,areal,ereal)*np.cos(oomegareal+theta1(tpreal,dt_vr,Mbhreal,areal,ereal))*Lm(perS2g,dthetaS2g)/keprad(tpreal,dt_vr,Mbhreal,areal,ereal)**2))

#The unit for the radial velocity is [pc/year]
radvelplot=-np.transpose(radvelplot)


#The length of tid and radvelplot has to correspond. They both have a length of 1000.
print('len(radvelplot) =',len(radvelplot))

plt.plot(tid+2003.44,radvelplot,color='fuchsia',label='Theoretical Radial / LOS Velocity  S4716',)
plt.xlabel('Time [yrs]')
plt.ylabel('RV [km/s]*1000')
plt.grid(alpha=0.5)
plt.ylim([-0.6,2.7])
